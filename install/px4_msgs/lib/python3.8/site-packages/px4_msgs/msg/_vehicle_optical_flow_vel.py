...
class VehicleOpticalFlowVel(metaclass=Metaclass_VehicleOpticalFlowVel):
    ...
    def __init__(self, **kwargs):
        ...
        if 'meas_gyro' not in kwargs:
            self.meas_gyro = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.meas_gyro = numpy.array(kwargs.get('meas_gyro'), dtype=numpy.float32)
            assert self.meas_gyro.shape == (3, )
    ...
    def __eq__(self, other):
        ...
        if all(self.ref_gyro != other.ref_gyro):
            return False
        if all(self.meas_gyro != other.meas_gyro):
            return False
        return True
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'gyro_bias' field must be a set or sequence with length 3 and each value of type 'float'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'ref_gyro' field must be a set or sequence with length 3 and each value of type 'float'"
    ...
    def meas_gyro(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float32, \
                "The 'meas_gyro' numpy.ndarray() must have the dtype of 'numpy.float32'"
            assert value.size == 3, \
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 True), \
                "The 'meas_gyro' field must be a set or sequence with length 3 and each value of type 'float'"
        self._meas_gyro = numpy.array(value, dtype=numpy.float32)
