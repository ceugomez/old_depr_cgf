  ...
  px4__msg__GimbalDeviceAttitudeStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4__msg__GimbalDeviceAttitudeStatus * data =
      (px4__msg__GimbalDeviceAttitudeStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4__msg__GimbalDeviceAttitudeStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          px4__msg__GimbalDeviceAttitudeStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
